...
export default function VoorraadSamenvoegenPage() {
  ...
  const saveMerged = () => {
    try {
      const baseName = draftName.trim() || 'voorraad-samenvoegen';
      const ts = new Date();
      const timestamp = formatTs(ts);
      const fileName = `${baseName}-${timestamp}.json`;
      
      // Save in format compatible with kelder-analyse
  ...
  if (isLoading) {
    return (
      <>
        <Head>
          <title>Voorraad Samenvoegen</title>
        </Head>
  ...
  return (
    <>
      ...
      <main style={{ padding: 16, maxWidth: 1200, margin: '0 auto' }}>
        <h1 style={{ fontSize: 24, fontWeight: 700, marginBottom: 8 }}>Voorraad Samenvoegen</h1>
        <p style={{ marginBottom: 16, color: '#6b7280' }}>
          Laad meerdere draft bestanden van kelder-inventaris en voeg ze samen tot één lijst. 
          ...
          <div style={modalBackdropStyle} onClick={() => setShowSaveModal(false)}>
            <div style={modalStyle} onClick={e => e.stopPropagation()}>
              ...
              <p style={{ marginTop: 0, marginBottom: 12, color: '#6b7280', fontSize: 14 }}>
                Geef een naam op voor dit bestand. Het bestand kan daarna gebruikt worden bij kelder-analyse.
              </p>
              <div style={{ display: 'grid', gap: 8 }}>
                <label style={labelStyle}>
                  ...
                  <input 
                    ...
                    placeholder="voorraad-samenvoegen"
                    style={inputStyle} 
                    autoFocus
                  />
                  {draftName && (
