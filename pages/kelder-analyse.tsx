...
function extractMerkFromName(name: string): string | null {
  if (!name) return null;
...
function computeMerk(row: AnalyseRow): { merk: string; fromName: boolean } {
  if (row.merk) {
...
export default function KelderAnalysePage() {
  ...
  void archiveLoading; // setArchiveLoading is used, archiveLoading kept for future use
  ...
  // Automatisch analyseren wanneer grouped data beschikbaar is en er nog geen analysed data is
  useEffect(() => {
    const barcodes = Object.keys(grouped);
    if (barcodes.length > 0 && analysed.length === 0 && upload) {
      // Start analyse automatisch na een korte delay om te zorgen dat grouped state is bijgewerkt
      const timer = setTimeout(() => {
        // Gebruik de grouped state direct uit de closure
        const barcodesToAnalyse = Object.keys(grouped);
        if (barcodesToAnalyse.length === 0) return;
        startAnalyse();
      }, 100);
  ...
  // searchArchivedFor - kept for future UI integration
  ...
  if (false) void searchArchivedFor;
  ...
  // openKeywordSearch - kept for future UI integration
  ...
  // Mark as used to satisfy TypeScript strict mode
  if (false) void openKeywordSearch;
  ...
  if (isLoading) {
    return (
      <>
        <Head><title>Inventaris analyse</title></Head>
  ...
  return (
    <>
      ...
      <main style={{ padding: 16, maxWidth: 1300, margin: '0 auto' }}>
        <h1 style={{ fontSize: 24, fontWeight: 700, marginBottom: 12 }}>Inventaris analyse</h1>
        ...
        <div style={{ display: 'flex', gap: 8, alignItems: 'center', flexWrap: 'wrap', marginBottom: 12 }}>
          <button onClick={onChooseFile} style={{ padding: '6px 10px', border: '1px solid #ccc', borderRadius: 4 }}>Upload inventaris</button>
          ...
          <table style={{ borderCollapse: 'collapse', width: '100%' }}>
            <thead style={{ background: '#f9fafb', position: 'sticky', top: 0, zIndex: 10 }}>
              <tr>
                ...
                <SortableTh label="Aantal" active={sortFound.key==='scanQty'} dir={sortFound.dir} onClick={() => setSortFound(prev => ({ key: 'scanQty', dir: prev.key==='scanQty' && prev.dir==='asc' ? 'desc' : 'asc' }))} />
