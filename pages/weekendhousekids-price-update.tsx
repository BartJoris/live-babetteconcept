...
export default function WeekendHouseKidsPriceUpdate() {
  ...
  // Parse Order Confirmation CSV (export-Order-*.csv)
  const parseOrderConfirmationCSV = async (text: string) => {
    ...
    if (headerLineIdx === -1) {
      throw new Error('Kan de header regel niet vinden in Order Confirmation CSV');
    }

    const headers = lines[headerLineIdx].split(';').map(h => h.trim());
    const styleIdx = headers.findIndex(h => h.toUpperCase() === 'STYLE');
    const referenceIdx = headers.findIndex(h => h.toUpperCase() === 'REFERENCE');
    const variantIdx = headers.findIndex(h => h.toUpperCase() === 'VARIANT');
    const srpIdx = headers.findIndex(h => h.toUpperCase() === 'SRP');

    if (referenceIdx === -1 || variantIdx === -1 || srpIdx === -1) {
      throw new Error('CSV mist verplichte kolommen: REFERENCE, VARIANT, SRP');
    }
    ...
    for (let i = headerLineIdx + 1; i < lines.length; i++) {
      ...
      // Check if this is a product name row (has STYLE but no REFERENCE)
      if (styleValue && !referenceValue) {
        // Skip product name rows
        continue;
      }

      // Check if this is a data row (has REFERENCE)
  ...
  return (
    <>
      <Head>
        <title>Weekend House Kids - Prijs Update</title>
      </Head>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                ...
                <div className={`border-2 ${orderProducts.length > 0 ? 'border-green-500 bg-green-50 dark:bg-green-900/30' : 'border-orange-500 bg-orange-50 dark:bg-orange-900/30'} rounded-lg p-6`}>
                  ...
                  <p className="text-xs text-gray-500 dark:text-gray-500 mb-3 font-medium">
                    Voorbeeld: "order-3116870-20260204.csv"
                  </p>
                  <input
                    type="file"
                    accept=".csv"
                ...
                <div className={`border-2 ${srpPrices.size > 0 ? 'border-green-500 bg-green-50 dark:bg-green-900/30' : 'border-blue-500 bg-blue-50 dark:bg-blue-900/30'} rounded-lg p-6`}>
                  ...
                  <p className="text-xs text-gray-600 dark:text-gray-400 mb-3">
                    Met REFERENCE, VARIANT, SRP kolommen
                  </p>
                  <p className="text-xs text-gray-500 dark:text-gray-500 mb-3 font-medium">
                    Voorbeeld: "export-Order-3116870-20260204.csv"
                  </p>
                  <input
                    type="file"
                    accept=".csv"
