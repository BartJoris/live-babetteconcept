...
const STORAGE_KEY = 'stocksaleRows';
const DISCOUNT = 0.70;

function calcSalePrice(originalPrice: number | null): number {
...
export default function StocksalePage() {
  ...
  const [rows, setRows] = useState<StocksaleRow[]>([]);
  ...
  useEffect(() => {
    try {
      ...
      if (raw) {
        const parsed = JSON.parse(raw) as StocksaleRow[];
  ...
  const totals = useMemo(() => {
    ...
    for (const r of rows) {
      totalItems += r.qty;
      if (r.currentPrice != null) totalCurrentValue += r.currentPrice * r.qty;
      totalNewValue += r.newPrice * r.qty;
    }
    return { totalItems, totalCurrentValue, totalNewValue, uniqueProducts: rows.length };
  }, [rows]);
  ...
  const handleLookup = async (barcode: string) => {
    ...
    try {
      ...
      if (data && data.found) {
        const salePrice: number | null = data.salePrice ?? null;
        setRows(prev => [{
          ...
          newPrice: calcSalePrice(salePrice),
  ...
  const handleUpdatePrices = async () => {
    const updates = rows
      .filter(r => r.productId != null && r.newPrice >= 0)
      .map(r => ({ variantId: r.productId as number, price: r.newPrice }));
    ...
    try {
      ...
      if (data.success) {
        ...
        setRows(prev => prev.map(r =>
          r.productId != null ? { ...r, currentPrice: r.newPrice } : r
        ));
      } else {
  ...
  const handlePrintLabels = () => {
    ...
    const labelRows: { name: string; barcode: string; oldPrice: string; newPrice: string }[] = [];
    for (const r of rows) {
      for (let i = 0; i < r.qty; i++) {
        labelRows.push({
          name: r.name,
          barcode: r.barcode,
          oldPrice: r.currentPrice != null ? `€${r.currentPrice.toFixed(2)}` : '',
          newPrice: `€${r.newPrice.toFixed(2)}`,
        });
      }
    }
    ...
    const labelsHtml = labelRows.map((l, idx) =>
      ...
      (l.oldPrice ? `<span class="op">${escapeHtml(l.oldPrice)}</span>` : '') +
      `<span class="np">${escapeHtml(l.newPrice)}</span>` +
    ...
    const html =
      ...
      `.op{position:absolute;top:1mm;right:0;font-size:8pt;color:#555;text-decoration:line-through}` +
      `.np{position:absolute;top:5mm;right:0;font-size:18pt;font-weight:900;line-height:1}` +
  ...
  const handleUpdateStock = async () => {
    const updates = rows
      ...
      .map(r => ({ productId: r.productId as number, newQuantity: r.qty }));

    if (updates.length === 0) {
      setAlert('Geen producten met geldige productId om voorraad bij te werken.', 'error');
      return;
    }
    ...
    try {
      ...
      if (data.success) {
        setAlert(`Voorraad bijgewerkt voor ${data.updatedCount} product(en).`, 'success');
        setRows(prev => prev.map(r =>
          r.productId != null ? { ...r, qtyAvailable: r.qty } : r
        ));
      } else {
        setAlert(`Voorraad bijwerken mislukt: ${data.error ?? 'Onbekende fout'}`, 'error');
      }
    } catch {
      setAlert('Voorraad bijwerken mislukt.', 'error');
    }
  ...
  if (isLoading) {
    return (
      <>
        <Head><title>Stocksale</title></Head>
  ...
  return (
    <>
      ...
      <main style={{ padding: 16, maxWidth: 1400, margin: '0 auto' }}>
        <h1 style={{ fontSize: 24, fontWeight: 700, marginBottom: 4 }}>Stocksale</h1>
        <p style={{ marginBottom: 16, color: '#6b7280' }}>
          Scan producten — de nieuwe prijs wordt automatisch berekend ({DISCOUNT * 100}% korting, afgerond).
          Pas individueel aan waar nodig, werk bij in Odoo en print labels.
        </p>
        ...
        <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap', alignItems: 'center', marginBottom: 16 }}>
          ...
          <button
            ...
            disabled={isUpdatingStock || rows.length === 0}
            style={{
              ...btnStyle,
              background: isUpdatingStock ? '#c4b5fd' : '#7c3aed',
              color: '#fff',
              border: '1px solid #7c3aed',
              cursor: isUpdatingStock || rows.length === 0 ? 'not-allowed' : 'pointer',
            }}
          >
            {isUpdatingStock ? 'Bijwerken...' : 'Voorraad bijwerken'}
          </button>
          <button
          ...
          <div style={{ marginLeft: 'auto', fontSize: 14, display: 'flex', gap: 16, flexWrap: 'wrap' }}>
            ...
            <span><strong>Huidige waarde:</strong> €{totals.totalCurrentValue.toFixed(2)}</span>
            <span><strong>Nieuwe waarde:</strong> €{totals.totalNewValue.toFixed(2)}</span>
          </div>
        </div>
          ...
          <table style={{ borderCollapse: 'collapse', width: '100%' }}>
            <thead style={{ background: '#f9fafb' }}>
              <tr>
                ...
                <th style={{ ...thStyle, textAlign: 'right' }}>Huidige prijs</th>
                <th style={{ ...thStyle, textAlign: 'right' }}>Voorraad</th>
                <th style={{ ...thStyle, textAlign: 'center', width: 80 }}>Gescand</th>
                <th style={{ ...thStyle, textAlign: 'right', background: '#fef3c7', width: 110 }}>Nieuwe prijs</th>
            ...
            <tbody>
              {rows.map((r, i) => (
                <tr key={`${r.barcode}-${i}`} style={{ borderTop: '1px solid #e5e7eb' }}>
                  ...
                  <td style={{ ...tdStyle, textAlign: 'right', background: '#fffbeb' }}>
            ...
            {rows.length > 0 && (
              <tfoot style={{ background: '#f9fafb', fontWeight: 700 }}>
                <tr style={{ borderTop: '2px solid #d1d5db' }}>
                  ...
                  <td style={{ ...tdStyle, textAlign: 'right' }}>€{totals.totalCurrentValue.toFixed(2)}</td>
                  <td style={tdStyle} />
                  <td style={{ ...tdStyle, textAlign: 'center' }}>{totals.totalItems}</td>
                  <td style={{ ...tdStyle, textAlign: 'right', background: '#fef3c7', fontWeight: 700 }}>
                    €{totals.totalNewValue.toFixed(2)}
                  </td>
                  <td style={tdStyle} />
                </tr>
              </tfoot>
