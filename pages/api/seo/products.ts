...
import { sortGalleryImages } from '../../../lib/seo/altText';
import type { SeoGalleryImageMeta, SeoProductItem } from '../../../lib/seo/types';

async function handler(
  req: NextApiRequestWithSession,
  res: NextApiResponse<SeoProductItem[] | { error: string }>
) {
  if (req.method !== 'GET') {
  ...
  try {
    ...
    const galleryRecords = await odooClient.searchRead<{
      id: number;
      name: string;
      sequence: number;
      ...
      'product.image',
      [['product_tmpl_id', 'in', templateIds]],
      ['id', 'name', 'sequence', 'product_tmpl_id'],
      100000,
      undefined,
      'sequence asc, id asc'
    );

    const galleryByTemplate = new Map<number, SeoGalleryImageMeta[]>();
    galleryRecords.forEach((img) => {
    ...
    const result: SeoProductItem[] = templates.map((t) => {
      const hasMainImage = hasImageSet.has(t.id);
      const galleryImages = sortGalleryImages(galleryByTemplate.get(t.id) ?? []);
      ...
      return {
        ...
        hasMainImage,
        imageCount: (hasMainImage ? 1 : 0) + galleryImages.length,
        galleryImages,
      };
    });
