...
async function handler(req: NextApiRequestWithSession, res: NextApiResponse) {
  ...
  try {
    ...
    console.log(`🏠 Parsing Weekend House Kids PDF: ${pdfFile.originalFilename}`);
    ...
    try {
      ...
      console.log(`✅ Extracted ${pdfText.length} characters from WHK PDF`);
    } catch (pdfError) {
    ...
    const srpMap = parseWeekendHouseKidsSrpFromText(pdfText);
    const priceMap: Record<string, number> = {};
    for (const [ref, rrp] of srpMap) {
      priceMap[ref] = rrp;
    }

    console.log(`✅ Extracted ${srpMap.size} SRP prices from Weekend House Kids PDF`);
    for (const [ref, rrp] of srpMap) {
    ...
    console.error('WHK PDF parsing error:', error);
