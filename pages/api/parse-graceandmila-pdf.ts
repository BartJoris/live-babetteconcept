...
import { extractGraceAndMilaProducts } from '@/lib/suppliers/graceandmila/pdf';
...
async function handler(req: NextApiRequestWithSession, res: NextApiResponse) {
  ...
  try {
    ...
    console.log(`📋 Parsing Grace & Mila PDF: ${fileName}`);
    ...
    const products = extractGraceAndMilaProducts(pdfText);

    if (products.length === 0) {
      return res.status(200).json({
        success: false,
        error: 'Geen producten gevonden in Grace & Mila factuur.',
